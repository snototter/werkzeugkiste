...
/// @brief Returns true if the given point is within the rectangle.
template <typename T>
...
class Circle_ {  // NOLINT(readability-identifier-naming)
 public:
  ...
  /// @brief Constructs an invalid circle.
  Circle_() = default;
  ...
  /// @brief Circle from 3 points.
  ///
  /// If they are collinear, the circle will be invalid.
  /// Check via `IsValid()` afterwards.
  Circle_(const vec_type& p, const vec_type& q, const vec_type& r);

  /// @brief Returns true if this is a valid circle.
  inline bool IsValid() const { return Sign(radius_) > 0; }

  /// @brief Returns the center point as 2d vector.
  vec_type Center() const { return center_; }

  /// @brief Returns the x-coordinate of the center point.
  T CenterX() const { return center_.X(); }

  /// @brief Returns the y-coordinate of the center point.
  ...
  /// @brief Returns true if the point is inside the circle. If you need to
  ...
  /// @brief Returns the number of points of tangency (i.e. 0, 1, or 2) between
  /// this circle and the given point. Optionally sets the points of tangency.
  int PointsOfTangency(const vec_type& pt,
      vec_type* pot1 = nullptr,
      vec_type* pot2 = nullptr) const;

  /// @brief Returns the number of direct common tangents (German "aeussere
  ...
  /// @brief Returns the number of transverse common tangents (German "innere
  ...
  /// @brief Returns the number of intersections (0, 1, or 2) of the two
  /// circles. Returns -1 if the circles are equal and thus every point on the
  /// circles is an intersection point. Optionally sets the intersection points.
  int IntersectionCircleCircle(const Circle_<T>& other,
      vec_type* intersection1 = nullptr,
      vec_type* intersection2 = nullptr) const;

  /// @brief Returns the number of intersections (0, 1, or 2) of this circle and
  /// the line. Optionally sets the intersection points.
  int IntersectionCircleLine(const Line2d_<T>& line,
      vec_type* intersection1 = nullptr,
      vec_type* intersection2 = nullptr) const;

  /// @brief Returns the number of intersections (0, 1, or 2) of this circle and
  /// the line segment(!). Optionally sets the intersection points.
...
/// @brief Available double-precision specialization for Circle_.
...
/// @brief Represents a line/segment in 2d Euclidean space.
template <typename T>
class Line2d_ {  // NOLINT(readability-identifier-naming)
 public:
  ...
  /// @brief Default constructor yields an invalid line/segment
  Line2d_() = default;

  /// @brief Construct a line from 2 real valued points. In case of a segment,
  /// these denote the start and end points.
  ...
  /// @brief For a segment, this returns the start point. For a line, it's
  /// simply one of the given points to construct the line in the first place.
  const vec_type& From() const { return pt_from_; }

  /// @brief Sets the first reference point (starting point of a line segment;
  /// or any point on a line).
  void SetFrom(const vec_type& from) { pt_from_ = from; }

  /// @brief For a segment, this returns the end point. For a line, it's simply
  /// one of the given points to construct the line in the first place.
  const vec_type& To() const { return pt_to_; }

  /// @brief Sets the second reference point (end point of a line segment; or
  /// any point on a line).
  void SetTo(const vec_type& to) { pt_to_ = to; }

  /// @brief Returns the length from the start to the end point. As such, it's
  /// only meaningful for a line segment.
  inline double Length() const { return Direction().Length(); }

  /// @brief Returns the non-normalized direction vector from the start point to
  /// the end point.
  inline vec_type Direction() const { return pt_from_.DirectionVector(pt_to_); }

  /// @brief Returns the unit direction vector from the start point to the end
  /// point.
  ...
  /// @brief Returns true if the line object is valid, *i.e.* start and end
  ...
  /// @brief Returns the angle between the line and the given directional
  /// vector. The angle will be between 0 and Pi.
  ...
  /// @brief Returns the angle between the line and the given directional
  /// vector. The angle will be between 0 and 180 degrees.
  ...
  /// @brief Returns the closest point on the line, i.e. the projection of the
  /// given point onto this line.
  vec_type ClosestPointOnLine(const vec_type& point) const;

  /// @brief Returns the point on this line segment(!) which is closest to the
  /// given point.
  vec_type ClosestPointOnSegment(const vec_type& point) const;

  /// @brief Returns the minimum distance between the point and this line.
  ...
  /// @brief Returns the minimum distance between the point and this segment.
  ...
  /// @brief Returns true if the two lines are collinear.
  ...
  /// @brief Returns true if the point is left of this line as specified by
  /// pt_from_ --> pt_to_.
  ///
  /// If you need to distinguish left-of vs. exactly on the line, pass a
  /// valid pointer `is_on_line`.
  bool IsPointLeftOfLine(const vec_type& point,
      bool* is_on_line = nullptr) const;

  /// @brief Returns true if this line intersects the other line and optionally
  /// sets the `intersection_point`.
  bool IntersectionLineLine(const Line2d_& other,
      vec_type* intersection_point = nullptr) const;

  /// @brief Returns true if this line(!) intersects the other line segment(!)
  /// and optionally sets the `intersection_point`.
  bool IntersectionLineLineSegment(const Line2d_& segment,
      vec_type* intersection_point = nullptr) const;

  /// @brief Returns true if this line segment(!) intersects the other line
  /// segment(!) and optionally sets the `intersection_point`.
  bool IntersectionLineSegmentLineSegment(const Line2d_& segment,
      vec_type* intersection_point = nullptr) const;

  /// @brief Returns the number of intersections (0, 1, or 2) of this line and
  /// the circle. Optionally sets the intersection points.
  int IntersectionLineCircle(const Circle_<T>& circle,
      vec_type* intersection1 = nullptr,
      vec_type* intersection2 = nullptr) const;

  /// @brief Returns the number of intersections (0, 1, or 2) of this line
  /// segment(!) and the circle. Optionally sets the intersection points.
  int IntersectionLineSegmentCircle(const Circle_<T>& circle,
      vec_type* intersection1 = nullptr,
      vec_type* intersection2 = nullptr) const;

  /// @brief Clips this line(!) such that only the part within the rectangle
  /// remains, check via `IsValid()` afterwards.
  Line2d_<T> ClipLineByRectangle(const vec_type& top_left,
      const vec_type& size) const;

  /// @brief Clips this line segment(!) such that only the part within the
  /// rectangle remains, check via `IsValid()` afterwards.
 ...
 private:
  /// @brief Starting point of a line segment or one direction-defining point on
  /// a line.
  vec_type pt_from_{0, 0};

  /// @brief Ending point of a line segment or the second direction-defining
  /// point on a line.
  vec_type pt_to_{0, 0};
};

/// @brief Available specialization for a 2-dimensional line/segment with
/// double precision.
...
/// @brief Represents a line/segment in 3d Euclidean space.
template <typename T>
class Line3d_ {  // NOLINT(readability-identifier-naming)
 public:
  ...
  /// @brief Returns the angle between the line and the plane.
  ...
  /// @brief Returns the point which is offset * Direction() away from the
  /// line's starting point, i.e. offset == 0 is the starting point, offset == 1
  /// is the end point.
...
/// @brief Available 3-dimensional double precision specialization of a
/// line/segment.
...
/// @brief Represents a plane in 3d Euclidean space.
template <typename T>
class Plane_ {  // NOLINT(readability-identifier-naming)
 public:
  ...
  /// @brief Constructs an invalid plane.
  Plane_() = default;

  /// @brief Constructs a plane from its Hessian form.
  Plane_(const vec_type& normal, T offset)
      : normal_{normal.UnitVector()}, offset_{offset} {}

  /// @brief Constructs a plane from 3 points. If the points are collinear, the
  /// plane will be invalid.
  Plane_(const vec_type& p, const vec_type& q, const vec_type& r);

  /// @brief Returns true if the plane has a valid normal vector.
  bool IsValid() const { return IsEpsEqual(normal_.LengthSquared(), 1.0); }

  /// @brief Returns the plane's normal vector.
  vec_type Normal() const { return normal_; }

  /// @brief Returns the plane's offset, i.e. the distance from the plane to
  ...
  /// @brief Returns the distance from the given point to the plane.
  inline T DistancePointToPlane(const vec_type& pt) const {
    return (normal_.Dot(pt) + offset_);
  }

  /// @brief Returns true if the given point is in front of the plane, i.e. the
  ...
  /// @brief Returns true if the given point lies exactly on the plane.
  inline bool IsPointOnPlane(const vec_type& pt) const {
    return IsEpsZero(DistancePointToPlane(pt));
  }

  /// @brief Returns the dihedral angle, *i.e.* the angle between the two
  /// planes, in radians. The angle will be between 0 and Pi.
  inline double AngleRad(const Plane_& other) const {
    return std::acos(std::max(-1.0, std::min(1.0, normal_.Dot(other.normal_))));
  }

  /// @brief Returns the dihedral angle, *i.e.* the angle between the two
  /// planes, in degrees. The angle will be between 0 and 180.
  inline double AngleDeg(const Plane_& other) const {
    return Rad2Deg(AngleRad(other));
  }

  /// @brief Returns the angle between this plane and the line in radians.
  /// The angle will be between -Pi/2 and Pi/2.
  inline double AngleRad(const Line3d_<T>& line) const {
    return line.AngleRad(*this);
  }

  /// @brief Returns the angle between this plane and the line in degrees.
  ...
  /// @brief Returns the plane's x-, y- and z-intercepts.
 ...
 private:
  /// @brief The plane's normal vector.
  vec_type normal_{0, 0, 0};

  /// @brief Distance between plane and origin of the coordinate system.
  T offset_{0};
};

/// @brief The default plane type which uses double precision.
