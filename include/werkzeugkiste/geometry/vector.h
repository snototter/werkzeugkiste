...
class Vec {
 public:
  ...
  /// @brief Default constructor initializes all fields with 0.
  Vec() noexcept {
  ...
  /// @brief Convenience (x, y) constructor for 2D vector.
  ...
  /// @brief Convenience (x, y, z) constructor for 3D vector.
  template <typename Tp = T>
  ...
  /// @brief Convenience (x, y, z, w) constructor for 4D vector.
  template <typename Tp = T>
  ...
  /// @brief Constructor accepting an `initializer_list` with
  ...
  /// @brief Convenience conversion to double precision.
  Vec<double, Dim> ToDouble() const {
    return static_cast<Vec<double, Dim>>(*this);
  }

  /// @brief Convenience conversion (truncation) to an integer vector.
  ...
  /// @brief Returns a vector with all coordinates set to `value`.
  ...
  /// @brief Returns the homogeneous representation of this vector, *i.e.* the
  /// vector has an additional dimension which is set to 1.
  ...
  /// @brief Supports array-like mutable access.
  ...
  /// @brief Array-like mutable access, which supports negative indexing.
  ...
  /// @brief Array-like non-mutable access, which supports negative indexing.
  /// For example, vec[-1] == vec[Dim - 1].
  ...
  /// @brief Holds the values of this vector.
  ...
  /// @brief "Almost equal" check for vectors with floating point value type.
  ///
  ...
  /// @brief Vectors with integral value type can be compared for equality.
  template <typename Tp = T>
  bool IsEqual(
  ...
  /// @brief Equality checks can be used for all vector specializations.
  ///
  ...
  /// @brief Inequality checks can be used for all vector specializations.
  friend bool operator!=(const Vec<T, Dim>& lhs, const Vec<T, Dim>& rhs) {
    return !(lhs == rhs);
  }
  ...
  /// @brief Performs element-wise addition and returns the modified instance.
  ...
  /// @brief Overloaded `vec1 + vec2`, see `AddVector`.
  Vec<T, Dim>& operator+=(const Vec<T, Dim>& rhs) { return AddVector(rhs); }

  /// @brief Overloaded `vec1 + vec2`. By-value passing of lhs helps optimizing
  ...
  /// @brief Adds the scalar to each dimension and returns the modified
  /// instance.
  ...
  /// @brief Overloaded `vec += scalar`, see `AddScalar`.
  Vec<T, Dim>& operator+=(T value) { return AddScalar(value); }

  /// @brief Overloaded `vec + scalar`.
  friend Vec<T, Dim> operator+(Vec<T, Dim> lhs, T rhs) {
    lhs += rhs;
    return lhs;
  }

  /// @brief Overloaded `scalar + vec`.
  friend Vec<T, Dim> operator+(T lhs, Vec<T, Dim> rhs) {
    rhs += lhs;
    return rhs;
  }

  /// @brief Performs element-wise subtraction and returns the modified
  /// instance.
  ...
  /// @brief Overloaded `vec1 -= vec2`, see `SubtractVector`.
  Vec<T, Dim>& operator-=(const Vec<T, Dim>& rhs) {
    return SubtractVector(rhs);
  }

  /// @brief Overloaded `vec1 -= vec2`.
  friend Vec<T, Dim> operator-(Vec<T, Dim> lhs, const Vec<T, Dim>& rhs) {
    lhs -= rhs;
    return lhs;
  }

  /// @brief Subtracts `value` from each dimension and returns the modified
  /// instance.
  ...
  /// @brief Overloaded `vec -= scalar`, see `SubtractScalar`.
  ...
  /// @brief Overloaded `scalar - vec`, which is `All(scalar) - vec`.
  ...
  /// @brief Multiplies each dimension by the given scalar and returns the
  /// modified instance.
  ...
  /// @brief Overloaded `vec *= scalar`, see `MultiplyScalar`.
  ...
  /// @brief Performs element-wise multiplication and returns the
  /// modified instance.
  ...
  /// @brief Overloaded `vec1 *= vec2`, see `MultiplyVector`.
  ...
  /// @brief Divides each element by the given scalar and returns the modified
  ...
  /// @brief Overloaded for convenience, see `DivideScalar`. Only supported
  ...
  /// @brief Overloaded for convenience. Divides each dimension by the given
  /// scalar. Only supported for floating point-based vector specializations.
  template <typename Tp = T>
  friend Vec<
      typename std::enable_if<std::is_floating_point<Tp>::value, Tp>::type,
      Dim>
  ...
  /// @brief Performs element-wise division and returns the modified
  ...
  /// @brief Overloaded element-wise division, see `DivideVector`. Only
  /// supported for floating point-based vector specializations.
  template <typename Tp = T>
  Vec<typename std::enable_if<std::is_floating_point<Tp>::value, Tp>::type,
      Dim>&
  ...
  /// @brief Returns a vector where each dimension is the result of the
  /// element-wise division `scalar / divisor[dimension]. Only supported for
  ...
  /// @brief Inverts (i.e. negates) each dimension.
  Vec<T, Dim>& Negate() {
    for (index_type i = 0; i < Dim; ++i) {
      val[i] *= -1;
    }
  ...
  /// @brief Returns a copy of this vector where each dimension is negated.
  ...
  /// @brief Returns the determinant of the two 2d vectors, i.e.
  ...
  /// @brief Returns the 3D vector cross product.
  template <typename Tp = T>
  ...
  /// @brief Returns a vector holding the absolute values of this vector.
  ...
  /// @brief Computes the L2 distance between this and the other.
  double DistanceEuclidean(const Vec<T, Dim>& other) const {
    auto diff = DirectionVector(other);
    return diff.Length();
  }

  /// @brief Computes the L1 distance between this and the other.
  ...
  /// @brief Returns the sum over all dimensions.
  T Sum() const {
    T sum{0};
    for (index_type i = 0; i < Dim; ++i) {
      sum += val[i];
  ...
  /// @brief Returns the 90° clock-wise rotated vector. This method assumes
  ...
  /// @brief Returns the 90° counter-clock-wise rotated vector. This method
  /// assumes that the coordinate system is right-handed and is only supported
  /// for 2D vectors.
  template <typename Tp = T>
  Vec<typename std::enable_if<(Dim == 2), Tp>::type, Dim>
  ...
  // TODO doc & test
  /// @brief Returns a copy of this vector rotated by the given radians.
  ///
  ...
  // TODO doc & test
  /// @brief Returns a double precision vector which is the result of
...
/// @brief Returns the length of the given polygon.
template <typename T, std::size_t Dim>
...
/// @brief Returns the length of the vector projection.
///
...
/// @brief Returns :math:`\operatorname{proj}_{\mathbf{b}} \mathbf{a}`, *i.e.*
...
/// @brief Computes the angle (in radians) of a 2d direction vector w.r.t. the
...
/// @brief Computes the angle (in degrees) of a 2d direction vector w.r.t. the
...
/// @brief Returns the unit direction vector given its angle (in
...
/// @brief Computes the minimum/maximum along each dimension.
///
...
// Add fmt formatter specialization for Vec in order to prevent SPDLOG implicit
// instantiation errors (with 0-dim vectors)
#if __has_include(<spdlog/fmt/fmt.h>)
#include <spdlog/fmt/fmt.h>

template <typename T, std::size_t Dim>
struct fmt::formatter<werkzeugkiste::geometry::Vec<T, Dim>>
    : fmt::formatter<std::string> {
  auto format(const werkzeugkiste::geometry::Vec<T, Dim>& vec,
