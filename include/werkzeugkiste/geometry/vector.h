...
public:
  ...
  template<typename Tp = T>
  inline const typename std::enable_if<(Dim > 1), Tp>::type& Y() const noexcept
  {
    return val[1];
  }
  ...
  template<typename Tp = T>
  inline const typename std::enable_if<(Dim > 2), Tp>::type& Z() const noexcept
  {
    return val[2];
  }

  template<typename Tp = T>
  inline const typename std::enable_if<(Dim > 3), Tp>::type& W() const noexcept
  {
    return val[3];
  }
  ...
  std::string ToString(bool include_type = true, int fixed_precision = 2) const
  {
    std::ostringstream s;
    if (include_type) {
...
template<typename T>
inline T Determinant(const Vec<T, 2>& a, const Vec<T, 2>& b)
{
...
/// See also: https://en.wikipedia.org/wiki/Vector_projection
template<typename T, std::size_t Dim>
inline Vec<T, Dim> VectorProjection(const Vec<T, Dim>& a, const Vec<T, Dim>& b)
{
  static_assert(std::is_floating_point<T>::value,
                "Vector type must be float or double!");
