...
template<typename _Tp>
inline Vec<_Tp, 3> CameraCenterFromRt(const Matrix<_Tp, 3, 3>& R,
                                      const Matrix<_Tp, 3, 1>& t)
{
...
/// Returns the optical center C = -R' * t.
template<typename _Tp>
inline Vec<_Tp, 3> CameraCenterFromRt(const Matrix<_Tp, 3, 4>& Rt)
{
  return EigenColToVec<_Tp, 3, 1>(-Rt.block(0, 0, 3, 3).transpose() * Rt.col(3),
                                  0);
}
...
template<typename _Tp>
inline Matrix<_Tp, 3, 3> GroundplaneToImageHomography(
    const Matrix<_Tp, 3, 4>& P)
{
  Matrix<_Tp, 3, 3> H;
...
/// projection matrix.
template<typename _Tp>
inline Matrix<_Tp, 3, 3> ImageToGroundplaneHomography(
    const Matrix<_Tp, 3, 4>& P)
{
...
template<typename _Tp>
inline Plane_<_Tp> ImagePlaneInCameraCoordinateSystem()
{
...
template<typename _Tp>
inline Plane_<_Tp> ImagePlaneInWorldCoordinateSystem(const Matrix<_Tp, 3, 3>& R,
                                                     const Vec<_Tp, 3>& t)
{
...
template<typename _Tp>
inline bool IsInFrontOfImagePlane(const Vec<_Tp, 3>& pt_world,
...
template<typename _Tp>
inline Line2d GetProjectionOfHorizon(const Matrix<_Tp, 3, 3>& K,
                                     const Matrix<_Tp, 3, 3>& R,
                                     const Vec<_Tp, 3>& t,
                                     const Vec2i& image_size = {0, 0})
{
  ...
  const Vec2d horizon_dir =
      Vec<_Tp, 2>(img_plane_normal[0], img_plane_normal[1]).UnitVector();
  if (IsEpsZero(horizon_dir[0]) && IsEpsZero(horizon_dir[1])) {
    ...
    std::tie(prj1, prj2) =
        ProjectToVecs(P,
                      Vec<_Tp, 3>(pt1[0], pt1[1], camera_center3d[2]),
                      Vec<_Tp, 3>(pt2[0], pt2[1], camera_center3d[2]));

    Line2d horizon(prj1, prj2);
    if ((image_size.Width() > 0) && (image_size.Height() > 0)) {
      return horizon
          .ClipLineByRectangle({0, 0},
                               static_cast<Line2d::vec_type>(image_size))
          .LeftToRight();
    } else {
...
// Field-of-View
template<typename _Tp>
inline bool IsPointInsideImage(const Vec<_Tp, 2>& pt, const Vec2i& img_size)
{
  return IsPointInsideRectangle<_Tp>(
      pt,
      Vec<_Tp, 2> {0.0, 0.0},
      Vec<_Tp, 2> {static_cast<_Tp>(img_size.Width()),
...
template<typename _Tp>
inline bool ProjectsPointOntoImage(const Vec<_Tp, 3>& pt_world,
                                   const Matrix<_Tp, 3, 4>& P,
